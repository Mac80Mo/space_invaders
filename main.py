...
class Game: # Spielfenster
    def __init__(self, width, height):
        ...
        self.background_img = pygame.image.load("spr_space_himmel.png") # Hintergrundbild

        while self.running: # Gameloop
            self.clock.tick(60)
            self.screen.blit(self.background_img, (0, 0))

            for event in pygame.event.get(): # Steuerung über Events
...
class Spaceship:
    def __init__(self, game, x, y):
        ...
        self.bullets = [] # Liste der Bullets/Geschosse

    def fire_bullet(self):
        self.bullets.append(Bullet(self.game, self.x, self.y)) # dieselben Werte wie vom Spaceship (Position)
